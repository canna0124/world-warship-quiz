...
async function onModeChange(){
  ...
  if(mode==="real"){
    ...
    if(modeData.wows.length){
      ...
      setStatus("WoWSは標準画像または公式HD画像を選べます。HDはWargaming API IDが必要です");
    }
  }
}
...
async function loadImage(item){
  ...
  try{
    ...
    img.classList.toggle("wows-lowres",item.mode==="wows" && !usingWowsHd);
...
async function wowsImageUrl(item){
  ...
  if(($("wowsImageQuality")?.value==="hd") && code){
