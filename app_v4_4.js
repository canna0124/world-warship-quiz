...
async function init(){
  ...
  modeData.warthunder=(wtData.items || []).map(x=>({...x,mode:"warthunder"}));
...
async function loadImage(item){
  ...
  img.classList.remove("wows-lowres","wows-medium","wows-hd","wt-image","gun-image","kancolle-banner-crop","kancolle-card-image");
